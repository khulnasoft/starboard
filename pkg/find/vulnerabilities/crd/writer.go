...
func (s *ReadWriter) createVulnerability(ctx context.Context, container string, report starboard.VulnerabilityScanResult, owner metav1.Object) (err error) {
	...
	vulnsSearch, err := s.client.KhulnasoftV1alpha1().VulnerabilityReports(namespace).List(ctx, metav1.ListOptions{
	...
	if len(vulnsSearch.Items) > 0 {
		...
		_, err = s.client.KhulnasoftV1alpha1().VulnerabilityReports(namespace).Update(ctx, deepCopy, metav1.UpdateOptions{})
	} else {
		...
		_, err = s.client.KhulnasoftV1alpha1().VulnerabilityReports(namespace).
...
func (s *ReadWriter) Read(ctx context.Context, workload kube.Object) (vulnerabilities.WorkloadVulnerabilities, error) {
	list, err := s.client.KhulnasoftV1alpha1().VulnerabilityReports(workload.Namespace).List(ctx, metav1.ListOptions{
		LabelSelector: labels.Set{
